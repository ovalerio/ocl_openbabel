...
//	        angle = 0.0; // doesn't explain why GetAngle is returning NaN but solves it for us;
  ...
  double OBForceFieldMMFF94GPU::EnergyElectrostatic(){
	    ...
	    ScalarType * dummy = new ScalarType[SIZE];
	    int dummy_indx = 0;
...
//		   std::cout << "path: " << full_path << std::endl;
	        ...
	        Event evtCalcPartials;
	        queue.enqueueNDRangeKernel(kernel, NullRange, global, local, NULL, &evtCalcPartials);
	        ...
	        // Launch second kernel to perform reduction (sum energies)
	        Kernel sumKernel(program, "sum");

	        // Create some extra buffers for scratch intermediate calculations and the final energy
	        const Buffer bufferEnergy = Buffer(context, CL_MEM_WRITE_ONLY, sizeof(ScalarType));
	        Buffer bufferInput = Buffer(context, CL_MEM_READ_ONLY, SIZE * sizeof(ScalarType));

	        // Copy the energies calculated in thre previous step to use as the input for the sum
//	        queue.enqueueCopyBuffer(bufferPartial, bufferInput, 0, 0, SIZE * sizeof(ScalarType));
	        queue.enqueueWriteBuffer(bufferInput, CL_TRUE, 0, SIZE * sizeof(ScalarType), dummy);
	        ...
	        // Set arguments to the sum kernel
//	        sumKernel.setArg(0, bufferPartial);
	        sumKernel.setArg(0, bufferInput);
	        sumKernel.setArg(1, cl::__local(SIZE * sizeof(ScalarType))); // allocate space for the scratch mem
	        sumKernel.setArg(2, SIZE);
	        sumKernel.setArg(3, bufferEnergy);

	        // Run the sum kernel on specific ND range
	        Event evtSumPartials;
	        queue.enqueueNDRangeKernel(sumKernel, NullRange, global, local, NULL, &evtSumPartials);
//
//	        // Read the energy after kernel completes execution
	        std::vector<Event> eventList;
	        eventList.push_back(evtSumPartials);
	        queue.enqueueReadBuffer(bufferEnergy, CL_TRUE, 0, sizeof(ScalarType), &energy, &eventList, NULL);
//	        ScalarType *scratch = new ScalarType[SIZE];
//	        queue.enqueueReadBuffer(bufferScratch, CL_TRUE, 0, SIZE * sizeof(ScalarType), scratch);
