...
namespace OpenBabel
{
  ...
  class OBForceFieldMMFF94: public OBForceField
  {
    ...
    public:
//      static int instances; // number of instances of this class (used for debugging purposes)
      //! Constructor
      explicit OBForceFieldMMFF94(const char* ID, bool IsDefault=true) : OBForceField(ID, IsDefault)
      {
        ...
        _gradientPtr = NULL;
        _grad1 = NULL;
        // NOTICE: just the first 7 characters compared.
        // Therefore using and ID like MMFF94sGPU or MMFF94GPU will still give the correct result
...
//		#ifdef _OPENMP // START OPENMP DEBUG
//		#pragma omp parallel // launch a parallel region with all available threads
//		{
//			#pragma omp master // only master enters here
//			{
//				instances++;
//				cout << "INSTANCE: "<< instances << "ID: " << ID << " - RUNNING THREADS = " << omp_get_num_threads() << endl;
//			}
...
//  int OBForceFieldMMFF94::instances = 0; // instances initial value would be zero
