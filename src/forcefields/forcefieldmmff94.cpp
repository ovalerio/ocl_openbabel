...
namespace OpenBabel
{
  ...
  double OBForceFieldMMFF94::Energy(bool gradients)
  {
    ...
    // FIXME HOW ABOUT A COLUMN FOR GRADIENTS !! THAT IS OBMINIMIZE
    ...
    timings[mk.TOTAL_BOND_CALCULATIONS] = _bondcalculations.size();
    timings[mk.TOTAL_ANGLE_CALCULATIONS] = _anglecalculations.size();
    timings[mk.TOTAL_STRBND_CALCULATIONS] = _strbndcalculations.size();
    timings[mk.TOTAL_TORSION_CALCULATIONS] = _torsioncalculations.size();
    timings[mk.TOTAL_OOP_CALCULATIONS] = _oopcalculations.size();
    timings[mk.TOTAL_VDW_CALCULATIONS] = _vdwcalculations.size();
    timings[mk.TOTAL_ELECTROSTATIC_CALCULATIONS] = _electrostaticcalculations.size();

    allocatedMemory[mk.MEM_BOND_CALCULATIONS] = sizeof(OBFFBondCalculationMMFF94) * _bondcalculations.size();
    allocatedMemory[mk.MEM_ANGLE_CALCULATIONS] = sizeof(OBFFAngleCalculationMMFF94) * _anglecalculations.size();
    allocatedMemory[mk.MEM_STRBND_CALCULATIONS] = sizeof(OBFFStrBndCalculationMMFF94) * _strbndcalculations.size();
    allocatedMemory[mk.MEM_TORSION_CALCULATIONS] = sizeof(OBFFTorsionCalculationMMFF94) * _torsioncalculations.size();
    allocatedMemory[mk.MEM_OOP_CALCULATIONS] = sizeof(OBFFOOPCalculationMMFF94) * _oopcalculations.size();
    allocatedMemory[mk.MEM_VDW_CALCULATIONS] = sizeof(OBFFVDWCalculationMMFF94) * _vdwcalculations.size();
    allocatedMemory[mk.MEM_ELECTROSTATIC_CALCULATIONS] = sizeof(OBFFElectrostaticCalculationMMFF94) * _electrostaticcalculations.size();
